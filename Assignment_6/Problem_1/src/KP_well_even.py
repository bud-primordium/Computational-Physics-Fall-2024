...
def compute_Vq_analytical(N, U0, Lw, Lb, a):
    ...
    Vq[non_zero] = (U0 / (2 * np.pi * n_list[non_zero] * 1j)) * (
        np.exp(-2j * np.pi * n_list[non_zero] * (a + Lb) / (2 * a))
        - np.exp(-2j * np.pi * n_list[non_zero] * (Lw) / (2 * a))
    )
    Vq[0] = U0 * Lb / a
    return Vq
...
def compute_Vq_fft(N, U0, Lw, Lb, a):
    ...
    V_x = np.where((x >= (Lw / 2)) & (x < ((a + Lb) / 2)), U0, 0)
...
def generate_potential_vectorized(x, a, Lw, U0_J):
    ...
    V_x = np.where(
        (pos_in_cycle >= (Lw / 2)) & (pos_in_cycle < ((2 * a - Lw) / 2)), U0_J, 0
    )
    return V_x
